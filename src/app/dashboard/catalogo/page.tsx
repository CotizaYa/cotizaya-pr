...
import {
  ...
  DoorOpen, Wind, Layers, Grid3x3, Wrench, Square,
} from 'lucide-react'
...
// ─────────────────────────────────────────────────────────────────
const CAT_ORDER = [
  'screen', 'puerta', 'ventana', 'closet',
  'aluminio', 'cristal', 'tornilleria', 'miscelanea',
] as const
...
// Accesorios de screen que se muestran en sub-sección separada
...
function CatalogoContent() {
  ...
  // Búsqueda desde 1 carácter (era 2 antes — bug corregido)
  ...
  const filteredAccessories = applySearch(accessoryProducts)
  const totalVisible        = filteredMain.length + filteredAccessories.length

  // ── Header sticky (compartido con skeleton y error) ─────────────
  const StickyHeader = (
    <div className="bg-white border-b border-gray-100 sticky top-0 z-20 w-full shadow-sm">
      ...
      <div className="flex items-center gap-2 px-3 pt-3 pb-2">
        ...
        <div className="relative flex-1 min-w-0">
          ...
          <input
            ...
            className="w-full h-10 pl-9 pr-4 bg-gray-100 rounded-xl text-sm font-medium outline-none focus:bg-white focus:ring-2 focus:ring-orange-300 transition-all"
            ...
            : sortedCats.map(cat => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`font-black tabular-nums ${active ? 'opacity-70' : 'text-gray-400'}`}>
                      {count}
                    </span>
                  </button>
                )
      ...
      <div className="flex-1 px-3 pt-3 pb-28 md:pb-8 w-full">
        {totalVisible === 0 ? (
          ...
          <>
            ...
            {/* Sub-sección: Materiales y Accesorios (solo en screen) */}
...
// ─────────────────────────────────────────────────────────────────
// Export default — Suspense necesario por useSearchParams
// ─────────────────────────────────────────────────────────────────
